...
const (
	...
	// NonPrivilegedMultiProcessAgentContainerName is the name of a multi process container
	// which may run any combination of core, trace and process agent processes.
	NonPrivilegedMultiProcessAgentContainerName AgentContainerName = "non-privileged-multi-process-agent"
...
const (
	...
	// Run non-privileged processes in a single container. If privileged process, for example `security-agent` or `system-probe`
	// is required, fall back to singleProcessContainer
	NonPrivilegedMultiProcessContainer ContainerProcessStrategyType = "nonPrivilegedMultiProcessContainer"
)
