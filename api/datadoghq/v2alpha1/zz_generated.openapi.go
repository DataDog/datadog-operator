		...
		Schema: spec.Schema{
			SchemaProps: spec.SchemaProps{
				...
				Properties: map[string]spec.Schema{
					...
					"labelsAsTags": {
						SchemaProps: spec.SchemaProps{
							Description: "LabelsAsTags maps Kubernetes labels to Datadog tags, scoped to the KSM check. Outer key is the Kubernetes resource kind (e.g. \"pod\", \"node\", \"deployment\"); inner map is label name -> Datadog tag name. Example:\n  labelsAsTags:\n    pod:\n      app: app\n    node:\n      zone: zone\nNote: the top-level `global.kubernetesResourcesLabelsAsTags` configures this at the agent level via environment variables. LabelsAsTags here writes into the KSM check instance config and applies only to KSM metrics.",
							Type:        []string{"object"},
							AdditionalProperties: &spec.SchemaOrBool{
								Allows: true,
								Schema: &spec.Schema{
					...
					"annotationsAsTags": {
						SchemaProps: spec.SchemaProps{
							Description: "AnnotationsAsTags maps Kubernetes annotations to Datadog tags, scoped to the KSM check. Outer key is the Kubernetes resource kind; inner map is annotation name -> Datadog tag name. Example:\n  annotationsAsTags:\n    pod:\n      tags_datadoghq_com_version: version\nAnnotation names must match the transformation done by kube-state-metrics (e.g. `tags.datadoghq.com/version` becomes `tags_datadoghq_com_version`). Note: the top-level `global.kubernetesResourcesAnnotationsAsTags` configures this at the agent level via environment variables. AnnotationsAsTags here writes into the KSM check instance config and applies only to KSM metrics.",
							Type:        []string{"object"},
							AdditionalProperties: &spec.SchemaOrBool{
								Allows: true,
								Schema: &spec.Schema{
