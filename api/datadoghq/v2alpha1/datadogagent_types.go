...
type KubeStateMetricsCoreFeatureConfig struct {
	...
	// Trade-off: the cluster-aggregate `.total` metrics
	// (kubernetes_state.{container,initcontainer}.<resource>_{requested,limit}.total)
	// are emitted with a reduced tag set that carries no host or node, so when
	// every node agent emits them the series collide at ingestion and the cluster
	// total under-reports; this is not recoverable at query time. Host-tagged
	// metrics such as kubernetes_state.pod.count are unaffected and can be
	// re-aggregated with `sum by (kube_cluster_name) without (host)` if needed.
	// Requires Agent / Cluster Agent 7.60+.
	//
