...
type KubeStateMetricsCoreFeatureConfig struct {
	...
	// The outer key is the Kubernetes resource kind (for example, "pod", "node", "deployment"),
	// and the inner map is label name -> Datadog tag name.
	// Example:
	//   labelsAsTags:
	//     pod:
	//       app: app
	...
	// The outer key is the Kubernetes resource kind, and the inner map is annotation name -> Datadog tag name.
	// Example:
	//   annotationsAsTags:
	//     pod:
	//       tags_datadoghq_com_version: version
	// Annotation names must match the transformation done by kube-state-metrics (for example, `tags.datadoghq.com/version` becomes `tags_datadoghq_com_version`).
	// NOTE: The top-level `global.kubernetesResourcesAnnotationsAsTags` configures this at the Agent level via environment variables. AnnotationsAsTags here writes to the KSM check instance config and applies only to KSM metrics.
	// +optional
