...
func (u *Updater) Start(log logr.Logger) error {
	...
	go func() {
		...
		rcClient.Subscribe(string(state.ProductDebug), func(update map[string]state.RawConfig, applyStateCallback func(string, state.ApplyStatus)) {
			...
			_ = serviceState
			u.enableFeatures(ctx, ddAgents, false) // serviceState)
			...
			*/
			...
			if err := u.upgradeAgents(ctx, ddAgents, "agent:7.49.0"); err != nil {
...
func (u *Updater) upgradeAgents(ctx context.Context, ddAgents []v2alpha1.DatadogAgent, image string) error {
	ddAgents, err := u.getUpdatableAgents(ctx)
	if err != nil {
		return err
	}

	for _, dda := range ddAgents {
		if err := u.upgradeAgentRollingUpdate(dda, image); err != nil {
			return err
		}
	}
...
func (u *Updater) upgradeAgentRollingUpdate(dd v2alpha1.DatadogAgent, image string) error {
...
func (u *Updater) monitorUpdate(dda v2alpha1.DatadogAgent) (*appsv1.DaemonSet, error) {
}

func (u *Updater) enableFeature(dd v2alpha1.DatadogAgent, state bool) error {
	newDD := dd.DeepCopy()
...
func (u *Updater) enableFeatures(ctx context.Context, ddAgents []v2alpha1.DatadogAgent, state bool) error {
	for _, dd := range ddAgents {
		if err := u.enableFeature(dd, state); err != nil {
			return err
		}
	}
