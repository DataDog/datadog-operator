...
func (r *Reconciler) reconcileInstanceV2(ctx context.Context, logger logr.Logger, instance *datadoghqv2alpha1.DatadogAgent) (reconcile.Result, error) {
	...
	for _, provider := range *providersList {
		...
		if provider.Name == kubernetes.OpenShiftRHCOSProvider {
			config := instance.Spec.Override[datadoghqv2alpha1.NodeAgentComponentName]
	...
	return r.updateStatusIfNeededV2(logger, instance, newStatus, result, nil)
}
...
// updateProvidersList updates the providers list with nodes that had previously not passed their node delay time
func (r *Reconciler) updateProvidersList(logger logr.Logger) {
	for nodeName := range *r.profiles.GetNewNodes() {
		nodeObj := &corev1.Node{}
		err := r.client.Get(context.TODO(), types.NamespacedName{Name: nodeName}, nodeObj)
		if err != nil {
			logger.Error(err, "Unable to get node to update providers list", "node", nodeName)
			continue
		}
		_ = r.profiles.SetProvider(nodeObj)
	}
}
