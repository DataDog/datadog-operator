...
1. **Builds a fake Kubernetes client** pre-populated with the provided DDA, DAPs, and the embedded `DatadogAgentInternal` CRD.
...
The binary is written to `bin/<platform>/operator-render`. The Makefile target also runs as part of `make generate-manifests` to keep the embedded DDAI CRD in sync with `config/crd/bases/v1/`.
...
Each DAP produces an additional DaemonSet scoped to the nodes matching the profile's affinity rules.
