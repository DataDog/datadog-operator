...
func TestHasUserPDB(t *testing.T) {
	...
	// A namespace-wide user PDB (empty selector matches every pod).
	emptyUserPDB := policyv1.PodDisruptionBudget{
		ObjectMeta: metav1.ObjectMeta{Name: "ns-wide", Namespace: "default"},
		Spec:       policyv1.PodDisruptionBudgetSpec{Selector: &metav1.LabelSelector{}},
	}
	// A more-specific user PDB requiring both app=x and env=prod.
	specificUserPDB := policyv1.PodDisruptionBudget{
		ObjectMeta: metav1.ObjectMeta{Name: "specific", Namespace: "default"},
		Spec:       policyv1.PodDisruptionBudgetSpec{Selector: &metav1.LabelSelector{MatchLabels: map[string]string{"app": "x", "env": "prod"}}},
	}
	// A user PDB that selects pods WITHOUT a `tier` label (MatchExpression).
	noTierUserPDB := policyv1.PodDisruptionBudget{
		ObjectMeta: metav1.ObjectMeta{Name: "no-tier", Namespace: "default"},
		Spec: policyv1.PodDisruptionBudgetSpec{Selector: &metav1.LabelSelector{
			MatchExpressions: []metav1.LabelSelectorRequirement{{Key: "tier", Operator: metav1.LabelSelectorOpDoesNotExist}},
		}},
	}
	...
	for _, tc := range []struct {
		...
		{name: "user PDB selects the pod", existing: []policyv1.PodDisruptionBudget{userPDB}, podLabels: map[string]string{"app": "x"}, want: true},
		{name: "temp PDB ignored", existing: []policyv1.PodDisruptionBudget{tempPDB}, podLabels: map[string]string{"app": "x"}, want: false},
		{name: "no pod labels", existing: []policyv1.PodDisruptionBudget{userPDB}, podLabels: nil, want: false},
		{name: "user PDB does not select the pod", existing: []policyv1.PodDisruptionBudget{userPDB}, podLabels: map[string]string{"app": "y"}, want: false},
		// A broader user PDB (app=x) covers a pod carrying app=x plus extra
		// labels — must be detected so we don't stack a second PDB on it.
		{name: "broader user PDB covers the pod", existing: []policyv1.PodDisruptionBudget{userPDB}, podLabels: map[string]string{"app": "x", "env": "prod"}, want: true},
		// A more-specific user PDB (app=x,env=prod) covers a pod carrying both.
		{name: "more-specific user PDB matches the pod", existing: []policyv1.PodDisruptionBudget{specificUserPDB}, podLabels: map[string]string{"app": "x", "env": "prod"}, want: true},
		// ...but must NOT be treated as covering a pod missing env=prod.
		{name: "more-specific user PDB does not match a partial pod", existing: []policyv1.PodDisruptionBudget{specificUserPDB}, podLabels: map[string]string{"app": "x"}, want: false},
		{name: "namespace-wide (empty selector) covers the pod", existing: []policyv1.PodDisruptionBudget{emptyUserPDB}, podLabels: map[string]string{"app": "x"}, want: true},
		// Accuracy over the old MatchLabels heuristic: a `tier DoesNotExist`
		// PDB must NOT be treated as covering a pod that HAS a tier label.
		{name: "MatchExpression PDB not selecting the pod is not detected", existing: []policyv1.PodDisruptionBudget{noTierUserPDB}, podLabels: map[string]string{"app": "x", "tier": "api"}, want: false},
	} {
		t.Run(tc.name, func(t *testing.T) {
			assert.Equal(t, tc.want, hasUserPDB(tc.existing, tc.podLabels))
		})
	}
}
