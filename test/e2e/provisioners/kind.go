...
type KubernetesProvisionerParams struct {
	...
	yamlWorkloadPaths []YAMLWorkload
}
...
// KubernetesProvisionerOption is a function that modifies the ProvisionerParams
...
func WithYAMLWorkload(yamlWorkload YAMLWorkload) KubernetesProvisionerOption {
	return func(params *KubernetesProvisionerParams) error {
		params.yamlWorkloadPaths = append(params.yamlWorkloadPaths, yamlWorkload)
		return nil
	}
}
...
func LocalKindRunFunc(ctx *pulumi.Context, env *environments.Kubernetes, params *KubernetesProvisionerParams) error {
	...
	// Setup DDA options
	if params.ddaOptions != nil && operatorComp != nil {
	...
	for _, workload := range params.yamlWorkloadPaths {
