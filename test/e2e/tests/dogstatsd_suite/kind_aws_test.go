...
type awsDogstatsdSuite struct {
	dogstatsdSuite
}

func TestAWSDogstatsdSuite(t *testing.T) {
...
`),
	...
	e2eOpts := []e2e.SuiteOption{
		e2e.WithStackName(fmt.Sprintf("operator-dsd-awskind-%s", strings.ReplaceAll(common.K8sVersion, ".", "-"))),
		e2e.WithProvisioner(provisioners.KubernetesProvisioner(provisionerOptions...)),
	}

	e2e.Run(t, &awsDogstatsdSuite{}, e2eOpts...)
}
