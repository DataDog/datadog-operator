...
// is non-empty. Severity is a static property of the issue type (see
// componenthealth.SeverityForIssueType), not tracked here.
...
func (r *ComponentHealthReconciler) reconcilePod(ctx context.Context, pod types.NamespacedName, component string, issues []componenthealth.DetectedIssue) {
	...
	// crash-looping containers); only presence per issue type matters here
	// (severity is a static property of the issue type, not of the detection).
