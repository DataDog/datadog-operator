...
func (r *Reconciler) internalReconcileV2(ctx context.Context, instance *datadoghqv1alpha1.DatadogAgentInternal) (reconcile.Result, error) {
...
func (r *Reconciler) reconcileInstanceV2(ctx context.Context, logger logr.Logger, instance *datadoghqv1alpha1.DatadogAgentInternal) (reconcile.Result, error) {
	var result reconcile.Result
	...
	// DDA controller is managing the token and providing it to DDAI thanks to `setDCATokenFromDDA`.
	// We have two cases:
	// 1. User provides a secret in DDA, same secret is provided to DDAI.
	// 2. User relies on secret automated generation in DDA (providing `global.clusterAgentToken` results in secret generation).
	r.ensureDCATokenInStatus(instance, newStatus, resourceManagers, logger)
...
func (r *Reconciler) updateStatusIfNeededV2(logger logr.Logger, agentdeployment *datadoghqv1alpha1.DatadogAgentInternal, newStatus *datadoghqv1alpha1.DatadogAgentInternalStatus, result reconcile.Result, currentError error, now metav1.Time) (reconcile.Result, error) {
	if currentError == nil {
...
func (r *Reconciler) setMetricsForwarderStatusV2(logger logr.Logger, agentdeployment *datadoghqv1alpha1.DatadogAgentInternal, newStatus *datadoghqv1alpha1.DatadogAgentInternalStatus) {
...
// The token secret name is based on the DDA name, so we need to use the DDA name.
// As the DDAI name is the DDA name with "-ddai" suffix, we need to remove it.
func getDefaultDCATokenSecretName(instance *datadoghqv1alpha1.DatadogAgentInternal) string {
	s := secrets.GetDefaultDCATokenSecretName(instance)
	return strings.Replace(s, "-ddai", "", 1)
}

func (r *Reconciler) ensureDCATokenInStatus(instance *datadoghqv1alpha1.DatadogAgentInternal, newStatus *datadoghqv1alpha1.DatadogAgentInternalStatus, resourceManagers feature.ResourceManagers, logger logr.Logger) {
	if instance.Status.ClusterAgent != nil && instance.Status.ClusterAgent.GeneratedToken != "" {
		// Already there; nothing to do.
		return
	}

	// The token is managed by the DDA controller, so we need to use the Reconciler client instead of the store.
	// Moreover, its name is based on the DDA name, so we need to use the DDA name.
	tokenSecret := &corev1.Secret{}
	expectedSecretName := getDefaultDCATokenSecretName(instance)
	if err := r.client.Get(context.TODO(), types.NamespacedName{
		Namespace: instance.Namespace,
		Name:      expectedSecretName,
	}, tokenSecret); err != nil {
		logger.V(1).Info("expected token secret was not found", "secretName", expectedSecretName)
		return
	}

	managedToken := tokenSecret.Data[common.DefaultTokenKey]
	if newStatus == nil {
	...
	newStatus.ClusterAgent.GeneratedToken = string(managedToken)
}

func (r *Reconciler) updateMetricsForwardersFeatures(dda *datadoghqv1alpha1.DatadogAgentInternal, features []feature.Feature) {
	if r.forwarders != nil {
