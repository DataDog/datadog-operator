...
func (c *OtelCollectorGatewayComponent) Reconcile(ctx context.Context, params *ReconcileComponentParams) (reconcile.Result, error) {
	...
	deployment := componentotelcollectorgateway.NewDefaultOtelCollectorGatewayDeployment(params.DDAI)
	podManagers := feature.NewPodTemplateManagers(&deployment.Spec.Template)

	// Set Global setting on the default deployment
	global.ApplyGlobalSettingsOtelCollectorGateway(params.Logger, podManagers, params.DDAI.GetObjectMeta(), &params.DDAI.Spec, params.ResourceManagers, params.RequiredComponents)
	...
	return c.reconciler.createOrUpdateDeployment(params.Logger, params.DDAI, deployment, params.Status, updateStatusV2WithOtelCollectorGateway)
}
...
func (c *OtelCollectorGatewayComponent) Cleanup(ctx context.Context, params *ReconcileComponentParams) (reconcile.Result, error) {
	deployment := componentotelcollectorgateway.NewDefaultOtelCollectorGatewayDeployment(params.DDAI)
	return c.reconciler.cleanupV2OtelCollectorGateway(params.Logger, params.DDAI, deployment, params.Status)
}

func (r *Reconciler) cleanupV2OtelCollectorGateway(logger logr.Logger, ddai *v1alpha1.DatadogAgentInternal, deployment *appsv1.Deployment, newStatus *v1alpha1.DatadogAgentInternalStatus) (reconcile.Result, error) {
...
func updateStatusV2WithOtelCollectorGateway(deployment *appsv1.Deployment, newStatus *v1alpha1.DatadogAgentInternalStatus, updateTime metav1.Time, status metav1.ConditionStatus, reason, message string) {
	newStatus.OtelCollectorGateway = condition.UpdateDeploymentStatus(deployment, newStatus.OtelCollectorGateway, &updateTime)
	condition.UpdateDatadogAgentInternalStatusConditions(newStatus, updateTime, common.OtelCollectorGatewayReconcileConditionType, status, reason, message, true)
}

func deleteStatusWithOtelCollectorGateway(newStatus *v1alpha1.DatadogAgentInternalStatus) {
	newStatus.OtelCollectorGateway = nil
	condition.DeleteDatadogAgentInternalStatusCondition(newStatus, common.OtelCollectorGatewayReconcileConditionType)
}
