...
// nodeProvider resolves the providers.Provider for ddai from its node-provider
// annotation, independent of the older cluster-provider/providercaps mechanism.
func nodeProvider(introspectionEnabled bool, ddai *v1alpha1.DatadogAgentInternal) providers.Provider {
	if !introspectionEnabled {
		return providers.Default()
	}
	return providers.Get(ddai.Annotations[kubernetes.NodeProviderAnnotationKey])
}
