...
func Test_gpuFeature_NodeAgentTracefsCapability(t *testing.T) {
	...
	t.Run("talos adds the tracefs volume in privileged mode", func(t *testing.T) {
	...
	t.Run("talos adds no tracefs volume outside privileged mode", func(t *testing.T) {
		f := &gpuFeature{}
		tmpl := newPodTemplate()
		mgr := feature.NewPodTemplateManagers(tmpl)
		require.NoError(t, f.ManageNodeAgent(mgr))
