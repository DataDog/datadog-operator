...
func (r *Reconciler) createOrUpdateDDAI(ctx context.Context, ddai *v1alpha1.DatadogAgentInternal) error {
...
func (r *Reconciler) addDDAIStatusToDDAStatus(ctx context.Context, status *v2alpha1.DatadogAgentStatus, ddai metav1.ObjectMeta) error {
	currentDDAI := &v1alpha1.DatadogAgentInternal{}
	if err := r.client.Get(ctx, types.NamespacedName{Name: ddai.Name, Namespace: ddai.Namespace}, currentDDAI); err != nil {
		if !apierrors.IsNotFound(err) {
			r.log.Error(err, "unexpected error during DDAI get")
