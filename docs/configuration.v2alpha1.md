...
| global.secretBackend.type | The built-in secret backend type to use (for example, `k8s.secrets`, `docker.secrets`, `aws.secrets`). Alternative to the `Command` parameter; when type is set, the Agent uses the built-in backend to resolve secrets. Requires Agent 7.70+. |
