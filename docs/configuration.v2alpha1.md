...
| features.apm.instrumentation.targets | A list of targets to apply the auto instrumentation to. The first target that matches the pod will be used. If no target matches, the auto instrumentation will not be applied. Full config key: apm_config.instrumentation.targets (Requires Cluster Agent 7.64.0+) |
