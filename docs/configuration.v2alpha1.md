...
| [key].containers.[key].startupProbe.exec.command | Command is the command line to execute inside the container, with the working directory set to root ('/') in the container's filesystem. The command is executed directly, not inside a shell, so traditional shell instructions ('|', etc) won't work. To use a shell, you must explicitly call the shell. An exit status of 0 is considered live/healthy, while a non-zero status indicates unhealthy. |
...
| [key].containers.[key].startupProbe.grpc.service | Service is the name of the service to place in the gRPC HealthCheckRequest. If this is not specified, the default behavior is defined by gRPC. More info: https://github.com/grpc/grpc/blob/master/doc/health-checking.md.|
| [key].containers.[key].startupProbe.httpGet.host | Host name used for connection, defaults to the pod IP. Consider setting "Host" in httpHeaders instead. |
...
| [key].containers.[key].startupProbe.tcpSocket.host | Optional: Host name used for connection, defaults to the pod IP. |
...
| [key].containers.[key].startupProbe.terminationGracePeriodSeconds | Optional duration in seconds for the pod to terminate gracefully upon probe failure. The grace period is the duration in seconds between when the processes running in the pod receive a termination signal and when they are forcibly halted with a kill signal. Set this value to be longer than the expected cleanup time for your process. If this value is nil, the pod's terminationGracePeriodSeconds will be used. Otherwise, this value overrides the value provided by the pod spec. Value must be non-negative integers. The value zero indicates stop immediately via the kill signal (no opportunity to shut down). This is a beta field and requires enabling ProbeTerminationGracePeriod feature gate. Minimum value is 1. spec.terminationGracePeriodSeconds is used if unset. |
| [key].containers.[key].startupProbe.timeoutSeconds | Number of seconds after which the probe times out. Defaults to 5 seconds. Minimum value is 1. More info: https://kubernetes.io/docs/concepts/workloads/pods/pod-lifecycle#container-probes |
