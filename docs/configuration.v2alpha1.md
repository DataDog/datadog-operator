...
| features.gpu.patchCgroupPermissions | PatchCgroupPermissions enables the patch of cgroup permissions for the GPU monitoring feature, in cases where the container runtime is not properly configured and the agent containers lose access the GPU devices Default: false |
