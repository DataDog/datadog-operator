...
var (
	image          string
	upgradeExample = `
  # upgrade the version of the datadog cluster agent to latest defined
  %[1]s upgrade

  # upgrade the version of the datadog cluster agent to latest defined in DatadogAgent foo
  %[1]s upgrade foo
...
func New(streams genericclioptions.IOStreams) *cobra.Command {
	...
	cmd.Flags().StringVarP(&image, "image", "i", "datadog/cluster-agent:latest", "The image of the Datadog Cluster Agent (default: datadog/cluster-agent:latest)")
...
func (o *options) run() error {
	...
	for _, dd := range ddList.Items {
		...
		if err != nil {
			...
			fmt.Println(fmt.Sprintf("Cluster Agent image updated successfully in %s/%s", dd.GetNamespace(), dd.GetName()))
		}
	}
	return nil
}
