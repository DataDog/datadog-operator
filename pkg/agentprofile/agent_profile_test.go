...
func TestApplyCreateStrategy(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "status changes from Waiting to InProgress - LastTransition updated",
			profilesByNode: map[string]types.NamespacedName{
		...
		{
			name: "nodes over limit with status change - LastTransition updated",
			profilesByNode: map[string]types.NamespacedName{
				"node1": {Namespace: testNamespace, Name: "profile1"},
				"node2": {Namespace: testNamespace, Name: "profile1"},
		...
		{
			name: "already labeled nodes with status change - LastTransition updated",
			profilesByNode: map[string]types.NamespacedName{
				"node1": {Namespace: testNamespace, Name: "profile1"},
				"node2": {Namespace: testNamespace, Name: "profile1"},
				"node3": {Namespace: testNamespace, Name: "profile1"},
		...
		{
			name: "multiple profiles - one changes status, one stays same",
			profilesByNode: map[string]types.NamespacedName{
				"node1": {Namespace: testNamespace, Name: "profile1"},
				"node2": {Namespace: testNamespace, Name: "profile1"},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Verify status and LastTransition behavior
			for _, profile := range tt.appliedProfiles {
				profileNSName := types.NamespacedName{Namespace: profile.Namespace, Name: profile.Name}
				if expectedCount, ok := tt.expectedNodesLabeled[profileNSName]; ok {
					assert.NotNil(t, profile.Status.CreateStrategy, "CreateStrategy should not be nil")
					...
					// Verify LastTransition behavior
					assert.NotNil(t, profile.Status.CreateStrategy.LastTransition, "LastTransition should always be set")

					if changed, exists := tt.expectLastTransitionChanged[profileNSName]; exists {
						if changed {
							// Status changed: LastTransition should be updated to a time after the initial
							assert.True(t, profile.Status.CreateStrategy.LastTransition.After(initialTransitionTime.Time),
								"LastTransition should be after initial time when status changes (got %v, initial was %v)",
								profile.Status.CreateStrategy.LastTransition, initialTransitionTime)
						} else {
							// Status unchanged: LastTransition should remain the same
							assert.Equal(t, initialTransitionTime, *profile.Status.CreateStrategy.LastTransition,
								"LastTransition should remain unchanged when status doesn't change")
						}
					}
				}
			}
