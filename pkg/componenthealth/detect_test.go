...
func TestDetectPodIssues(t *testing.T) {
	tests := []struct {
		...
		{
			// Regression: LastTerminationState persists for the pod's entire
			// lifetime, so a container that recovered and is now Running must not
			// be flagged from that historical OOMKill, or the issue would never
			// resolve without the pod being deleted.
			name: "oomkilled container that has since recovered is not flagged",
			status: corev1.PodStatus{ContainerStatuses: []corev1.ContainerStatus{{
				Name:                 "cluster-agent",
		...
		{
			// Regression: RestartCount is cumulative and monotonically
			// non-decreasing, so a container that is currently Running must not
			// stay flagged as crash-looping purely off historical restarts, or the
			// issue would never resolve without the pod being deleted.
			name: "high restart count container that has since recovered is not flagged",
			status: corev1.PodStatus{ContainerStatuses: []corev1.ContainerStatus{{
				Name:         "cluster-agent",
				RestartCount: 7,
				State:        corev1.ContainerState{Running: &corev1.ContainerStateRunning{}},
...
func TestSeverityForIssueType(t *testing.T) {
	tests := []struct {
		issueType string
		want      Severity
	}{
	...
	for _, tt := range tests {
		assert.Equal(t, tt.want, SeverityForIssueType(tt.issueType))
	}
}
