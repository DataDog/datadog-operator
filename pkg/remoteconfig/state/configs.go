...
var allProducts = []string{ProductDebug}
...
// DebugConfigs returns the currently active APM configs
func (r *Repository) DebugConfigs() map[string]DebugConfig {
	typedConfigs := make(map[string]DebugConfig)

	configs := r.getConfigs(ProductDebug)

	for path, conf := range configs {
		// We control this, so if this has gone wrong something has gone horribly wrong
		typed, ok := conf.(DebugConfig)
		if !ok {
